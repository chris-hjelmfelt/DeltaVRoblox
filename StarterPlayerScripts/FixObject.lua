...
myObject = Player:WaitForChild('PlayerGui',100):WaitForChild('Math',100).FixObject.Broken
currentChild = myObject:FindFirstChild('Background')

-- Recreate Math object
...
-- Creates a new broken object to fix with Math
	...
	for i = 1, #child do
		if child[i].Name == 'Broken' then 			
			child[i].MouseButton1Click:connect(function()
				...
				local problem = Player:WaitForChild('PlayerGui', 100):WaitForChild('Math', 100).FixObject.Problem	
...
game.ReplicatedStorage.MathAnsweredCorrectly.OnClientEvent:Connect(function()  -- comes from PlayerAddedMisc script
	Player.PlayerGui.Math.FixObject.Problem.Visible = false
	-- Destroy this button and replace it 
...
function makeMathObject()
	...
	local pieces = myObject:GetChildren()
	for i = 1, #pieces do
		if pieces[i].Name == "Broken" or pieces[i].Name == "Ok" or pieces[i].Name == "R1" or pieces[i].Name == "R2" or pieces[each].Name == "R3" then
			-- Destroy this button and replace it 
			local place = pieces[i].Position
			local size =  pieces[i].Size
			child[i].Parent = nil
...
-- FixObject DoAnother Gui Buttons
Player:WaitForChild('PlayerGui'):WaitForChild('Math').FixObject.DoAnother.Yes.MouseButton1Click:connect(function()
	-- Prepare and display another broken object
	Player.PlayerGui.Math.FixObject.Broken.Visible = false	
	Player.PlayerGui.Math.FixObject.DoAnother.Visible = false
	Player.PlayerGui.Math.FixObject.WaitForIt.Visible = true
	CreateBrokenObject()
	activateBrokenObject()
	Player.PlayerGui.Math.FixObject.Broken.Visible = true
	Player.PlayerGui.Math.FixObject.WaitForIt.Visible = false					
end)
Player:WaitForChild('PlayerGui'):WaitForChild('Math').FixObject.DoAnother.No.MouseButton1Click:connect(function()	
	Player.PlayerGui.Math.FixObject.Broken.Visible = false	
	Player.PlayerGui.Math.FixObject.DoAnother.Visible = false
